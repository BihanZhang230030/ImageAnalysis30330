...
Tracking::Tracking(System *pSys,
        ...
        mnInitialFrameId(0), mbCreatedMap(false),
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    mImDepth = imDepth;
    // Step 3：构造Frame
//    cout << "GrabImageRGBD    bug 1 "   << endl;
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
    {
        if (mSensor == System::RGBD)
            mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,true,mpCamera);
        else if(mSensor == System::IMU_RGBD)
            mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,true,mpCamera,&mLastFrame,*mpImuCalib);
    }
    else
    {
        if (mSensor == System::RGBD)
            mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,false,mpCamera);
        else if(mSensor == System::IMU_RGBD)
            mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,false,mpCamera,&mLastFrame,*mpImuCalib);
    }
//    if(mCurrentFrame.N < 500)
//        return mLastFrame.GetPose();
    mCurrentFrame.mNameFile = filename;
    mCurrentFrame.mnDataset = mnNumDataset;
//    cout << "GrabImageRGBD    bug 2 "   << endl;
#ifdef REGISTER_TIMES
    vdORBExtract_ms.push_back(mCurrentFrame.mTimeORB_Ext);
#endif
    // Step 4：跟踪
...
//            cout << "Track bug 6.4" << endl;
#ifdef REGISTER_TIMES
            std::chrono::steady_clock::time_point time_StartNewKF = std::chrono::steady_clock::now();
#endif
            // 判断是否需要插入关键帧
            bool bNeedKF = NeedNewKeyFrame();
//            cout << "Track bug 6.5" << endl;
            ...
            if(bNeedKF && (bOK || (mInsertKFsLost && mState==RECENTLY_LOST &&
                                   (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO|| mSensor == System::IMU_RGBD))))
            {
//                cout << "Track bug 6.5.1" << endl;
//                CreateNewKeyFrame();  // 创建关键帧，对于双目或RGB-D会产生新的地图点
                mCurrentFrame.add();
//                cout << "Track bug 6.5.2" << endl;
                CreateNewKeyFrame();
            }
//            cout << "Track bug 6.6" << endl;
...
/// 2024-9-26
    ...
    int nmatches = matcher.SearchWithGMS(mpReferenceKF,mCurrentFrame,vpMapPointMatches);
...
{
//    cout << "TrackWithMotionModel    bug 1 "   << endl;
    ...
    // Step 1：更新上一帧的位姿；对于双目或RGB-D相机，还会根据深度值生成临时地图点
    UpdateLastFrame();
//    cout << "TrackWithMotionModel    bug 2 "   << endl;
...
//    std::cout<<" Extract ORB num : "<<mCurrentFrame.N<<std::endl;
//    cout << "TrackWithMotionModel    bug 3 "   << endl;
    int nmatches = matcher.SearchByProjectionWithOF(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR);
//    cout << "TrackWithMotionModel    bug 4 "   << endl;
...
//        cout << "TrackWithMotionModel    bug 4 "   << endl;
    Optimizer::PoseOptimization(&mCurrentFrame);
//        cout << "TrackWithMotionModel    bug 5 "   << endl;
    // Discard outliers
    // Step 5：剔除地图点中外点
...
//        cout << "TrackWithMotionModel    bug 6 "   << endl;
...
//    cout << "TrackLocalMap start " << endl;
    ...
    UpdateLocalMap();
//    cout << "TrackLocalMap finish " << endl;
    /// 2024-9-26
    return true;
    /// 2024-9-26
...
/// 2024-9-26
    // 相比ORB-SLAM2多了c3,c4
    if(((c1a||c1b||c1c) && c2)||c3 ||c4|| cnt < 200 || mCurrentFrame.mvKeys.size() < 400)
    {
...
{
//    cout << "Tracking::CreateNewKeyFrame bug 1" << endl;
    ...
    /// 2024-9-26
//    cout << "Tracking::CreateNewKeyFrame bug 2" << endl;
    last_kf = pKF;
    local_queue.emplace(pKF);
//    cout << "Tracking::CreateNewKeyFrame bug 3" << endl;
    while (local_queue.size() > 5)
        local_queue.pop();
//    cout << "Tracking::CreateNewKeyFrame bug 4" << endl;
//    cout << "Tracking::CreateNewKeyFrame GetMapPointMatches == " <<  pKF->GetMapPointMatches().size() <<  endl;
    for (int i = 0; i < pKF->GetMapPointMatches().size();i ++)
    {
//        cout << "Tracking::CreateNewKeyFrame bug 4.1" << endl;
        auto mp = pKF->GetMapPoint(i);
//        cout << "Tracking::CreateNewKeyFrame bug 4.2" << endl;
        if (mp != nullptr)
        {
//            cout << "Tracking::CreateNewKeyFrame bug 4.3" << endl;
            mp->AddObservation(pKF, i);
//            cout << "Tracking::CreateNewKeyFrame bug 4.4" << endl;
        }
    }
//    cout << "Tracking::CreateNewKeyFrame bug 5" << endl;
    /// 2024-9-26
...
//    cout << "Tracking::CreateNewKeyFrame bug 6" << endl;
    if(mpLastKeyFrame)
    {
