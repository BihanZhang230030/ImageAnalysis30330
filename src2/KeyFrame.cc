...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false), frameSize(F.frameSize)
{
    pyr_ = F.pyr_;
...
cv::Mat KeyFrame::GetPoseMat()
{
    unique_lock<mutex> lock(mMutexPose);
    cv::Mat mTcw_Mat;
    cv::eigen2cv(mTcw.matrix(),mTcw_Mat);
