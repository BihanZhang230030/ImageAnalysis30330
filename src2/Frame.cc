...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mTcw(frame.mTcw), mbHasPose(false), mbHasVelocity(false),frameSize(frame.frameSize)
{
    /// 2024-9-26
    pyr_ = frame.pyr_;
...
//    mvKeys.reserve(pts.size());
    ...
    UndistortKeyPoints();
    //todo 上面没有提取新的特征点，但是更新了位置，这里需要重新计算mvDepth和mvuRight
    ComputeStereoFromRGBD(imdepth);
//    count1 ++ ;
//    cv::imwrite("/home/xin/catkin_ws/vslam_ws/vins_ws/src/ORB_SLAM3_detailed_comments-master/image/_"+ to_string(count1) +"_" + to_string(mTimeStamp) + ".png",imdepth);
    mVw.setZero();
...
void Frame::add()
{
    ...
    if (pt_num < 1500)
    {
        ...
        (*mpORBextractorLeft)(image,cv::Mat(),add_pts,add_des,vLapping);
